...
from app.models import (
    Order, OrderLine, OcrSubmission, SapSyncLog,
    Customer, Product
)
from app.schemas import (
    OcrSubmissionRequest, OcrSubmissionResponse,
    SapRegisterRequest, SapRegisterResponse,
    SapSyncLogResponse,
)
...
def submit_ocr_data(
    submission: OcrSubmissionRequest,
    ...
    # OCR取込ログ作成
    status = "success" if failed_records == 0 else ("partial" if processed_records > 0 else "failed")
...
def list_ocr_submissions(
    ...
    """OCR取込ログ一覧取得"""
    submissions = db.query(OcrSubmission).order_by(
        OcrSubmission.submission_date.desc()
...
def list_sap_logs(
    ...
    """SAP連携ログ一覧取得"""
    logs = db.query(SapSyncLog).order_by(
        SapSyncLog.executed_at.desc()
    ).offset(skip).limit(limit).all()
    return logs
