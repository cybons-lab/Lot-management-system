...
export function ProductDetailPage() {
  ...
  const handleUpdate = useCallback((data: { maker_part_code: string; product_name: string; base_unit: string; consumption_limit_days?: number | null }) => {
    if (!makerPartCode) return;
    const updateData: ProductUpdate = { product_name: data.product_name, base_unit: data.base_unit, consumption_limit_days: data.consumption_limit_days };
    ...
    return (
      <div className={styles.root}>
        <div className={styles.emptyState.container}>
          ...
          <p className={styles.emptyState.description}>指定されたメーカー品番「{makerPartCode}」は存在しないか、削除されています。</p>
  ...
  return (
    <div className={styles.root}>
      ...
      <div className="rounded-lg border bg-white p-6 shadow-sm">
        {isEditing ? (
          <ProductForm product={product} onSubmit={handleUpdate} onCancel={() => setIsEditing(false)} isSubmitting={isUpdating} />
        ) : (
          <div className="space-y-4">
            <div className={styles.form.field}><label className={styles.form.label}>メーカー品番</label><p className="font-mono text-lg font-medium">{product.maker_part_code}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>商品名</label><p className="text-lg">{product.product_name}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>単位</label><p>{product.base_unit}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>消費期限日数</label><p>{product.consumption_limit_days ?? "-"}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>作成日時</label><p className="text-gray-600">{new Date(product.created_at).toLocaleString("ja-JP")}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>更新日時</label><p className="text-gray-600">{new Date(product.updated_at).toLocaleString("ja-JP")}</p></div>
          </div>
        )}
      </div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>商品「{product.product_name}」（{product.maker_part_code}）を削除します。この操作は取り消せません。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
