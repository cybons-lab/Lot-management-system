...
const warehouseTypeLabels: Record<string, string> = { internal: "社内", external: "外部", supplier: "仕入先" };
...
export function WarehouseDetailPage() {
  ...
  const handleUpdate = useCallback((data: { warehouse_code: string; warehouse_name: string; warehouse_type: string }) => {
    if (!warehouseCode) return;
    const updateData: WarehouseUpdate = { warehouse_name: data.warehouse_name, warehouse_type: data.warehouse_type };
    updateWarehouse({ warehouseCode, data: updateData }, { onSuccess: () => setIsEditing(false) });
    ...
    return (
      <div className={styles.root}>
        <div className={styles.emptyState.container}>
          ...
          <p className={styles.emptyState.description}>指定された倉庫コード「{warehouseCode}」は存在しないか、削除されています。</p>
  ...
  return (
    <div className={styles.root}>
      ...
      <div className="rounded-lg border bg-white p-6 shadow-sm">
        {isEditing ? (
          <WarehouseForm warehouse={warehouse} onSubmit={handleUpdate} onCancel={() => setIsEditing(false)} isSubmitting={isUpdating} />
        ) : (
          <div className="space-y-4">
            <div className={styles.form.field}><label className={styles.form.label}>倉庫コード</label><p className="font-mono text-lg font-medium">{warehouse.warehouse_code}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>倉庫名</label><p className="text-lg">{warehouse.warehouse_name}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>タイプ</label><p>{warehouseTypeLabels[warehouse.warehouse_type] ?? warehouse.warehouse_type}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>作成日時</label><p className="text-gray-600">{new Date(warehouse.created_at).toLocaleString("ja-JP")}</p></div>
            <div className={styles.form.field}><label className={styles.form.label}>更新日時</label><p className="text-gray-600">{new Date(warehouse.updated_at).toLocaleString("ja-JP")}</p></div>
          </div>
        )}
      </div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>倉庫「{warehouse.warehouse_name}」（{warehouse.warehouse_code}）を削除します。この操作は取り消せません。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
