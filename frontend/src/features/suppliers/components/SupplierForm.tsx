  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={formStyles.grid}>
      <div className={formStyles.field}>
        <Label htmlFor="supplier_code" className={formStyles.label}>仕入先コード <span className="text-red-500">*</span></Label>
        <Input id="supplier_code" {...register("supplier_code")} placeholder="例: SUP-001" disabled={isEditMode} className={formStyles.input} />
        {errors.supplier_code && <p className={formStyles.error}>{errors.supplier_code.message}</p>}
      </div>

      <div className={formStyles.field}>
        <Label htmlFor="supplier_name" className={formStyles.label}>仕入先名 <span className="text-red-500">*</span></Label>
        <Input id="supplier_name" {...register("supplier_name")} placeholder="例: サンプル商社" className={formStyles.input} />
